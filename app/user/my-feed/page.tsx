...
import { getPostsByUser, getPostsByCommunity, reactToPost, deletePost, updatePost, type Post } from "@/lib/api/posts";
...
export default function MyFeed() {
  ...
  const loadData = async () => {
    try {
      ...
      if (communityFilter && communityFilter !== "all") {
        // Filter by specific community but only show user's posts
        try {
          const allPosts = await getPostsByCommunity(communityFilter);
        ...
        data = await getPostsByUser(user!._id).catch(() => []);
      }
      setPosts(data);
    } catch (error) {
  ...
  const handleFilterChange = (communityId: string) => {
    ...
    router.push(`/user/my-feed${params.toString() ? `?${params.toString()}` : ""}`);
  };
    ...
    return (
      <div className="min-h-screen app-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-[var(--ink-muted)]">Loading posts...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen app-bg py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-3xl font-bold text-[var(--foreground)]">My Posts</h1>
            <p className="text-[var(--ink-muted)]">Posts you've created</p>
          </div>
          <Link
            href="/user/add-post"
                ...
                >
                  ...
                  <div className="p-4 pb-3">
                    <div className="flex items-start gap-3">
                      ...
                      {/* Edit/Delete Buttons (all posts are user's own in My Posts page) */}
